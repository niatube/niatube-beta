  ...
  async function handleAccess(
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        setMessage(
          result.error ||
            "Financial administrator access could not be verified."
        );
...
if (
  ...
  setMessage(
    "Your administrator account is not authorized to access the Financial Control Center.",
  );
...
window.location.assign(
  "/financial",
      ...
      setMessage(
       "The Financial Control Center access request could not be completed. Please try again."
      );
    ...
    <main className="min-h-screen bg-gray-50 px-6 py-16">
      <section className="mx-auto max-w-md rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
        <p className="text-sm font-black uppercase tracking-wide text-yellow-600">
          NiaTube Financial Control Center
        </p>
